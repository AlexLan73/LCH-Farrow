 ...
 * @brief Примеры использования OpenCL Singleton Manager
 * 
 * ДЕМОНСТРИРУЕТ:
 * 1. Инициализацию OpenCLManager один раз
 * 2. Создание нескольких GeneratorGPU (переиспользуют контекст)
 * 3. Program cache в действии
 * 4. Performance улучшения
 * 5. Error handling
 */

 namespace opencl_singleton_ex {
...
void Example1_BasicInitialization() {
    ...
    try {
        // Инициализировать OpenCL один раз
...
void Example2_MultipleGenerators() {
    ...
    try {
        ...
        // Создать несколько GeneratorGPU объектов
...
void Example5_ErrorHandling() {
    ...
    try {
        ...
        // Это должно выбросить исключение
        GeneratorGPU gen(params);
    }
...
║    Демонстрация использования OpenCLManager с GeneratorGPU    ║
╚════════════════════════════════════════════════════════════════╝
)" << std::endl;
